...
#
# 2026/09/12
# MultimodalCombinedImageMaskDatasetGenerator.py
...
class MultimodalImageMaskDatasetGenerator:
  ...
  def normalize_combined_image(self, combined_image):
    image = combined_image//len(self.NII_FILES)
  ...
  def generate_one(self, case_index, image_nii_filepaths, seg_nii_filepath,
       ...
       mh, mw, num_mask_slices = mask_data.shape
       print(mask_data.shape)
       ...
       for i in range(len(image_nii_filepaths)):
         ...
         # Compare the number of image_slices and num_mask_slices. 
       ...
       for slice_index in range(num_mask_slices):
          ...
          if valid == True:
            # Create an empty black combined_image
            combined_image = np.zeros((mh, mw), dtype=np.float32)
            for image_data in all_images_data:
              combined_image += image_data[:,:,slice_index]

            image = self.normalize_combined_image(combined_image)

            filename = str(case_index) + "_" + str(slice_index) + ".png"
            mask_filepath = os.path.join(output_masks_dir, filename)
            cv2.imwrite(mask_filepath, mask)
            print("Saved a mask", mask_filepath)

            image_filename = str(case_index) + "_" + str(slice_index) + ".png"
            image_filepath = os.path.join(output_images_dir, image_filename)
            cv2.imwrite(image_filepath, image)
            print("Saved a combined image", image_filepath)
...
if __name__ == "__main__":
  try:
    ...
    output_dir = "./UCSF-PDGM-Multimodal-Combined-Subset"
